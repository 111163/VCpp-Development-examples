...
// 使用组合框列举磁盘目录Dlg.cpp : 实现文件
//

#include "stdafx.h"
#include "使用组合框列举磁盘目录.h"
#include "使用组合框列举磁盘目录Dlg.h"
...
C使用组合框列举磁盘目录Dlg::C使用组合框列举磁盘目录Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(C使用组合框列举磁盘目录Dlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void C使用组合框列举磁盘目录Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBOBOXEX1, m_ComboEx);
}

BEGIN_MESSAGE_MAP(C使用组合框列举磁盘目录Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
END_MESSAGE_MAP()
...
// C使用组合框列举磁盘目录Dlg 消息处理程序

BOOL C使用组合框列举磁盘目录Dlg::OnInitDialog()
{
...
void C使用组合框列举磁盘目录Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR C使用组合框列举磁盘目录Dlg::OnQueryDragIcon()
{
...
void C使用组合框列举磁盘目录Dlg::LoadSysDisk(void)
{
