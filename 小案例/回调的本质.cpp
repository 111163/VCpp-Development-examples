...
// 就是说函数指针的位置已经做好了，但是这个位置要实现什么功能，还是要看用哪个回调函数了(也就是功能多样，并不明确，但在当时设计的时候，就要知道这个位置
// 应该是丰富的功能，而不是单一，固化的功能(如果是单一而固化，直接调用就行，无需经过函数指针这一步))
// 函数指针本身就说明这个位置到底用那种功能，在设计的时候并不明朗, 应该后来定制